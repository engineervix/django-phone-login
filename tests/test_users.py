...
class PhoneTokenTest(TransactionTestCase):
    def test_check_auth_user_model(self):
        user = User.objects.create_user(phone_number="+01 9212128291", username="iraycd", password="random-password")
    ...
    def test_phone_number_equals(self):
        ...
        user = User.objects.create_user(phone_number=phone_number, username="iraycd", password="random-password")
        self.assertIsNotNone(user)
