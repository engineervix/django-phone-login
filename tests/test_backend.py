...
class PhoneBackendTest(TransactionTestCase):
    ...
    def test_authenticate_login(self):
        ...
        user = phonebackend.create_user(phone_token=phone_token)
        self.assertEqual(user.phone_number, phone_number, "User creation")
        authenticated_user = phonebackend.authenticate(
            request=self.factory.get(""), pk=phone_token.id, otp=phone_token.otp
        )
        self.assertEqual(authenticated_user.phone_number, phone_number, "Login phonenumber")
