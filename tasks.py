...
def execute_bump_hack(c):
    ...
    2. git add pyproject.toml phone_login/__init__.py
    ...
    if release_type == "none":
        ...
        c.run(
            "git add pyproject.toml phone_login/__init__.py",
            pty=True,
        )
...
def run(c, version=False, plain=False, help=False):
    ...
    if version:
        c.run("python phone_login/__main__.py -v", pty=True)
    elif plain:
        c.run("python phone_login/__main__.py -p", pty=True)
    elif help:
        c.run("python phone_login/__main__.py -h", pty=True)
    else:
        c.run("python phone_login/__main__.py", pty=True)
